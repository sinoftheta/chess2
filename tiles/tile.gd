...
var visual_position:Vector2:
	get(): return (%AnimationOffset as Node2D).global_position + 0.5 * size + Vector2(0,-8)
	...
	set(value):
		...
		match value:
			...
			Constants.BoardID.shop:
				size = Vector2(58,50)
			Constants.BoardID.sell:
				size = Vector2(64,64)
		if not is_node_ready(): return
		(%Play as Node2D).visible = value == Constants.BoardID.play
		(%Shop as Sprite2D).visible = value == Constants.BoardID.shop
		(%Sell as Sprite2D).visible = value == Constants.BoardID.sell
var order:int
var logical_position:Vector2i:
	set(value):
		logical_position = value
		position = Vector2(
			logical_position.x * size.x,
			logical_position.y * size.y,
		)
...
func _on_ready() -> void:
	## here we reassign values that update nodes
	type = type
	chevron_animation = 0
	board_id = board_id
...
func _process(delta: float) -> void:
	...
	var t:int = Engine.get_frames_drawn()
	t += board_id * 16
	
	(%AoeHighlight as Sprite2D).modulate.a = 0.8 + 0.2 * sin(
		sin(t * 0.08 + logical_position.x * 0.5 + logical_position.y * 0.5)
	)
	
	(%HoverTarget as Sprite2D).scale = Vector2.ONE * (1.0 + 0.2 * sin(t * 0.08))
	
	(%SwayOffset as Node2D).position = Vector2(
		sin(t * 0.04 + logical_position.x * 0.5 + logical_position.y * 0.5),
		cos(t * 0.08 + logical_position.y * 0.5 + logical_position.x * 0.5),
	) * sway_amplitude
	
	(%MouseBounce as Node2D).position.y = cos(
		(t - mouse_bounce_ts) * 0.8
