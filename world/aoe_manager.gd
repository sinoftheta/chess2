...
func _ready() -> void:
	...
	SignalBus.animate_unit_aoe_on .connect(_on_animate_unit_aoe_on)
	SignalBus.animate_unit_aoe_off.connect(_on_animate_unit_aoe_off)
...
func _process(delta: float) -> void:
	...
	if center_tile:
		...
		#%PlayBoard.modulate = Color(.45,.45,.45)
	#else:
		#%PlayBoard.modulate = Color.WHITE
	#
	#%PlayTileManager.modulate = %PlayBoard.modulate
...
func _on_animate_unit_aoe_on(unit:Unit) -> void:
	pass
func _on_animate_unit_aoe_off() -> void:
	pass
